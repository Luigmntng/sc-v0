    ...
    axios.get(url)
        .then(({
           data
            ...
            let dati = {
            img_url: $(b).find('> a > img').attr('data-src')
            }
            hasil.push(dati)
            })
            resolve(hasil)
...
function kusonimeinfo(url) {
    ...
    .then((res) => {
        const $ = cheerio.load(res.data)
const hasil = {}
hasil.title = $('#venkonten > div.venzone > div.venser > div.post-thumb > h1').text()
hasil.info = $('#venkonten > div.venzone > div.venser > div.venutama > div.lexot > div.info').text()
hasil.thumb = $('#venkonten > div.venzone > div.venser > div.post-thumb > img').attr('src')
resolve(hasil)
})
})
}
...
module.exports = { facebook, dojindsgetimg, nhgetimg, nhentai, nhentaisearch, kusonimelatest, kusonimeinfo, otakudesuongoing, otakudesuinfo, sektekomiksearch, nekopoisearch, nekopoi, nekopoilatest, porno, doujindesuch, doujindesu, doujindesusearch, mangatoons, webtoons, ssweb, ssweb2, otakudesu, gore, hentai, quotes, doujindesulatest, quotesAnime, igdl, textpro, igstory, tiktok, ttdown, twitter, joox, pin }
